...
const solve = input => {
    parseCosts(input);
    const oreTarget = 1000000000000;
    let fuelNeeded = 1;
    let lastOreUse = 0;

    while (lastOreUse < oreTarget) {
        lastOreUse = getOreFor(fuelNeeded, 'FUEL');
        fuelNeeded += Math.ceil(fuelNeeded * (oreTarget - lastOreUse)/oreTarget);
    }
...
console.log(solve(test), 82892753);
...
176 ORE => 6 VJHF`;
console.log(solve(test), 5586022);
...
console.log(solve(test), 460664);

test = `3 JQFM, 5 QMQB, 20 WQCT => 8 PHBMP
2 XJFQR => 1 WQCT
133 ORE => 3 KFKWH
...
console.log(solve(test), 1935265);
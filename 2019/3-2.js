...
const mapCheck = (x, y, steps) => {
    if (!(x in map))
        map[x] = [];

    if (map[x][y] && (x != 0 || y != 0)) {
        crossings.push(map[x][y] + steps);
    }

}

const mapTrace = (x, y, direction, distance, mapFn, steps) => {
    let [newX, newY,  newSteps] = [x, y, steps + distance];
    switch (direction) {
        case 'R':
            newX += distance;
            while (distance --> 0) mapFn(x + distance, y, steps + distance);
            break;
        case 'L':
            newX -= distance;
            while (distance --> 0) mapFn(x - distance, y, steps + distance);
        ...
        case 'D':
            newY -= distance;
            while (distance --> 0) mapFn(x, y - distance, steps + distance);
            break;
        case 'U':
            newY += distance;
            while (distance --> 0) mapFn(x, y + distance, steps + distance);
    ...
    return [newX, newY, newSteps];
}


const parseStep = step => {
    let direction = step[0];
    let distance = parseInt(step.split(/\D/)[1], 10);

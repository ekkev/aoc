...
const mapCheck = (x, y) => {
    ...
    if (map[x][y] == 1 && (x != 0 || y != 0)) {
        crossings.push(Math.abs(x) + Math.abs(y));
    }

}

const mapTrace = (x, y, direction, distance, mapFn) => {
    let [newX, newY] = [x, y];
    switch (direction) {
        case 'R':
            newX += distance;
            while (distance --> 0) mapFn(x + distance, y);
            break;
        case 'L':
            newX -= distance;
            while (distance --> 0) mapFn(x - distance, y);
        ...
        case 'D':
            newY -= distance;
            while (distance --> 0) mapFn(x, y - distance);
            break;
        case 'U':
            newY += distance;
            while (distance --> 0) mapFn(x, y + distance);
...
const solve = input => {
    ...
    wire1.forEach(step => {
        ...
        [x, y] = mapTrace(x, y, direction, distance, mapMark);
    });

    [x, y] = [0, 0];
    wire2.forEach(step => {
        let [direction, distance] = parseStep(step);
        [x, y] = mapTrace(x, y, direction, distance, mapCheck);
    });

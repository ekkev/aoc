...
const solve = program => {
    ...
    const yrangeAtX = new Map;
    const findYRangeAtX = x => {
        if (!yrangeAtX.has(x)) {
            let firstY, lastY;
            let y = 0;
            while (++y) {
                const cpu = Intcpu(program);
                const hit = cpu.runUntilOutput([x, y]).output;

                if (hit && firstY === undefined)
                    firstY = y;
                ...
                if (!hit && lastY) {
                    yrangeAtX.set(x, [firstY, lastY]);
                    break;
                }
            }
        }

        return yrangeAtX.get(x);
    }

    const canFitSquareAt = (x, side) => {
        side--;
        let [topA, bottomA] = findYRangeAtX(x);
        // console.log(x, side, topA, bottomA);
        let [topB, bottomB] = findYRangeAtX(x + side);
        // console.log(x, side, topB, bottomB);
        if (topB <= bottomA - side) {
            return [x, topB];
        } else {
            return false;
        }
    ...
    let squareSide = 100;
    let min = squareSide*2,
        max = squareSide*squareSide;

    while (1) {
        let x = Math.floor((min+max)/2);
        if (canFitSquareAt(x, squareSide)) {
            max = x;
        } else {
            min = x;
        }

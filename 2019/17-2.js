...
const turns = { [Turn.RIGHT]: [Dir.UP, Dir.RIGHT, Dir.DOWN, Dir.LEFT, Dir.UP] };
turns[Turn.LEFT] = [...turns[Turn.RIGHT]].reverse()
...
const recordMap = program => {
    ...
    cpu.runWithIO(handleOutput);
    prettyPicture(map, v => String.fromCharCode(v) + ` `);
    return [map, robotPos];
}

const tracePath = (map, pos) => {
    const getMapRow = y => map.has(y) ? map.get(y) : map.set(y, new Map).get(y);
    const getMap = ([x, y]) => getMapRow(y).get(x);
    ...
    while (1) {
        if (getMap(moveOneStep(pos, getDirectionAfterTurn(direction, Turn.RIGHT))) === SCAFFOLD) {
            direction = getDirectionAfterTurn(direction, Turn.RIGHT);
            lastTurn = Turn.RIGHT;
        } else if (getMap(moveOneStep(pos, getDirectionAfterTurn(direction, Turn.LEFT))) === SCAFFOLD) {
            direction = getDirectionAfterTurn(direction, Turn.LEFT);
            lastTurn = Turn.LEFT;
        } else {
            return movements;
        }
        ...
        steps = 0;
        while (getMap(moveOneStep(pos, direction)) === SCAFFOLD) {
            pos = moveOneStep(pos, direction);
            steps++;
        }
        movements.push([lastTurn, steps]);
    }

}

const findMinSequences = (sequence, subs = []) => {
    if ((new Set(subs)).size > 3)
        return;

    if (!sequence.length)
        return subs;

    // Reasonable sequence length 2 to 6 to fit in 20char limit
    for (let len = 2; len <= 6; len++) {
        let s = findMinSequences(sequence.substr(len), [...subs, sequence.substr(0, len)]);
        if ((new Set(s)).size === 3) {
            return s;
        }
    }
...
const findSequences = movements => {
    const map = new Map;
    let id = 0;
    const toId = movement => map.has(movement) ? map.get(movement) : map.set(movement, id++).get(movement);
    const simplified = movements.map(m => toId(m.join(',')));
    const sequences = findMinSequences(simplified.join(''));
    const functionSequences = [...(new Set(sequences))];

    const recall = [...map.keys()];
    const routine = sequences.map(v => functionSequences.indexOf(v)).map(v => String.fromCharCode('A'.charCodeAt() + v)).join(',');
    const functions = functionSequences.map(fn => fn.split('').map(v => recall[v]).join(','));
    return [routine, ...functions];
}

const runSequences = (program, sequences) => {
    const cpu = Intcpu(program);
    const toSend = sequences.join('\n') + '\nn\n';
    const userInputs = toSend.split('').map(v => v.charCodeAt());

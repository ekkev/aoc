...
const solve = (min: number, start: XY, end: XY) => {
    let minute = min;
    let posq: XY[] = [start];
    do {
    ...
    } while (posq.length && minute++ < 4000)
}


let one = solve(0, [0, -1], [targetX, targetY + 1])!;
console.log(one);

let two = solve(one, [targetX, targetY + 1], [0, -1])!;
console.log(two);

let three = solve(two, [0, -1], [targetX, targetY + 1]);
console.log(three);
// 
// 290
// 562

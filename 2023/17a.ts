...
const itFinder = findPathsFlexi<[XY, DirStr, number]>({
    ...
    nextMovesFn: ([pos, dir, cnt]) => [
        [xymove(pos, dir), dir, cnt+1],
        [xymove(pos, xydirturn(dir, 'r')), xydirturn(dir, 'r'), 0],
        [xymove(pos, xydirturn(dir, 'l')), xydirturn(dir, 'l'), 0],
    ],
    costFn: ([pos], cost) => cost + matrixGet(mvalue, pos)!,
    cacheKeyFn: ([pos, dir, cnt]) => [pos, dir, cnt].join(),
    isValidMoveFn: ([pos, _dir, cnt]) => inMatrix(matrix, pos) && cnt <= 2,
});
